...
// IMPORTER getIO à la place de 'io'
...
          canAddParticipants: ['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(userRole),
          canRemoveParticipants: ['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(userRole),
...
    if (!adder || !['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(adder.role)) {
...
          canAddParticipants: ['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(user.role),
          canRemoveParticipants: ['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(user.role),
          canSendMessages: true,
        },
      });
    });
...
      await chat.save();
      
      const io = getIO(); // UTILISATION DE getIO()
...
    if (!remover || !['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(remover.role)) {
      return res.status(403).json({
        success: false,
        message: t('permissions_insuffisantes', lang),
      });
...
      const tache = await TacheThemeFormation.findById(entityId).populate('RESPONSABLE-FORMATION').lean();
      if (tache) {
...
    }
    
    const isCreatorOrAdmin = chat.createdBy.toString() === userId.toString() || chat.participants.some(p => p.user.toString() === userId.toString() && ['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(p.role));
...
    const isCreatorOrAdmin = chat.createdBy.toString() === userId.toString() || ['SUPER-ADMIN', 'ADMIN', 'RESPONSABLE-FORMATION'].includes(currentUser.role);
    
